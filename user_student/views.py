...
def spoken_english(request, pk, pk1, programName):
    ...
    scores_level = {}
    scores_module = {}
    scores_level_keys = [score.level_id.module.module_name + '-' + score.level_id.level_description for score in
                         program_scores]
    scores_module_keys = [score.level_id.module.module_name for score in program_scores]

    all_scores = {str(score.level_id.module.module_name) + '-' + str(score.level_id.level_description) + '-' + str(
        score.assessment_type_id.assessment_type): round(((score.user_score / score.total_score) * 100), 2) for score in
                  program_scores}
    scores_level = {}
    ...
    if len(program_modules) > 0:
        return render(request, "spoken_english.html",
                      {"m": program_modules, "pk": pk, "pk1": pk1, "pk2": programId, "p": programObj[0],
                       "l": program_levels, 'scores': all_scores})
    else:
        return render(request, 'error.html', {"pk": pk, "pk1": pk1})
...
def module_view(request, pk, pk1, pk2, pk3):
    ...
    return render(request, "module_view.html", {"l": levels, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "m": module,
...
def level_view(request, pk, pk1, pk2, pk3, pk4):
    ...
    return render(request, "level_view.html",
                  {"question_types": question_types, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, 'pk4': pk4,
                   "l": level})
...
def word_find(request, pk, pk1, pk2, m, l, narrative, assessment_type_id):
    ...
    for i in QandA:
        if ((i.question.level == level and i.question.level.module == module) and (
                i.question.question_type == q_type and i.question.narrative == narrative)):
    ...
    return render(request, "wordsearch/wordfind.html",
                  {"pk": pk, "pk1": pk1, "pk2": pk2, "m": module, "l": level, 'ans': ANS, 'typ': typ,
                   'narrative': narrative, 'question_content_id': question_content_id})


def list_narrative(request, pk, pk1, pk2, m, l,
                   assessment_type_id):  # returns hyperlinks which contains questions related to specify narratives
    print('list_narrative', assessment_type_id)
    level = module_level.objects.get(pk=l)
    module = program_module.objects.get(pk=m)
    assess_name = assessment_type.objects.get(assessment_type_id=assessment_type_id)
    print(assess_name)
    Qj = question.objects.filter(level=level,
                                 assessment_type=assessment_type_id)  # making sure we are queryting match the following quetions
    print(Qj)
    ...
    return render(request, "all_hyperlink.html",
                  {"pk": pk, "pk1": pk1, "pk2": pk2, "m": module, "l": level, 'd': distinct_narratives,
                   'assessment_type': assess_name, 'assessment_type_id': assessment_type_id})
...
def score_save(request, pk, pk1, pk2, m, l, typ, score, total_score):
    ...
    if (score == 0):
        pass_status = False
    if (str(q_type.assessment_type).lower() == 'text test' or str(q_type.assessment_type).lower() == 'video' or str(
            q_type.assessment_type).lower() == 'audio'):
        question_content_id = request.session.get('question_content_id')
        score_save_helper(student_id, q_type.assessment_type, level_id, batch_id, pass_status, score, total_score,
                          question_content_id, request.session.get('narrative'), typ)
    else:
        if (str(q_type.assessment_type).lower() == 'general assessment'):  # GA
            ...
            score_save_helper(student_id, q_type.assessment_type, level_id, batch_id, pass_status, score, total_score,
                              question_content_id, narrative, typ)
    return render(request, "score_card.html",
                  {'score': request.POST.get('user_score', 0), "pk": pk, "pk1": pk1, "pk2": pk2, "m": m, "l": l,
                   'narrative': narrative, 'pass_status': pass_status, 'typ': typ,
                   'question_type': str(q_type.assessment_type).lower()})


def score_save_helper(student_id, question_type_name, level_id, batch_id, pass_status, score, total_score,
                      question_content_id, narrative, typ):
    try:
        if (str(question_type_name).lower() == 'general assessment'):
            ...
            student_query = scores.objects.get(batch_id=batch_id, student_id=student_id, level_id=level_id,
                                               assessment_type_id=assessment_type_id)
            print('a1')
        elif (str(question_type_name).lower() == 'text test' or str(question_type_name).lower() == 'audio' or str(
                question_type_name).lower() == 'video'):
            assessment_type_id = assessment_type.objects.get(assessment_type__iexact=str(question_type_name).lower())
    ...
    if (student_query == None):
        if (str(question_type_name).lower() == 'general assessment'):
            print('a5')
            assessment_type_id = assessment_type.objects.get(assessment_type__iexact=str(question_type_name).lower())
            obj = scores.objects.create(student_id=student_id, batch_id=batch_id, level_id=level_id, user_score=score,
                                        total_score=total_score, date_time=datetime.datetime.now(),
                                        assessment_type_id=assessment_type_id, question_content_id=question_content_id)
            obj.save()
        elif (str(question_type_name).lower() == 'text test' or str(question_type_name).lower() == 'audio' or str(
                question_type_name).lower() == 'video'):
            assessment_type_id = assessment_type.objects.get(assessment_type__iexact=str(question_type_name).lower())
            ...
            obj = scores.objects.create(student_id=student_id, batch_id=batch_id, level_id=level_id, user_score=score,
                                        total_score=total_score, date_time=datetime.datetime.now(),
                                        question_content_id=question_content_id, assessment_type_id=assessment_type_id)
            obj.save()
        else:
            print('a7')
...
def match(request, pk, pk1, pk2, m, l, narrative, assessment_type_id):
    ...
    return render(request, "match/match25.html",
                  {"pk": pk, "pk1": pk1, "pk2": pk2, "m": module, "l": level, "typ": typ, 'cola': rans, 'colq': QUEST,
                   'two_cols': two_cols, 'final_options': final_options, 'question_content_id': question_content_id,
                   'narrative': narrative})
...
def crossword(request, pk, pk1, pk2, m, l, narrative, assessment_type_id):
    ...
    return render(request, "crossword/crossword.html",
                  {"pk": pk, "pk1": pk1, "pk2": pk2, "m": module, "l": level, 'nd_array': nd_array, 'legend': legend,
                   'cords': cords, 'across_or_down': across_or_down, 'items': items, 'answer_start': answer_start,
                   'answer_start_index': answer_start_index, 'answers': ans, 'new_cells_allowed': new_cells_allowed,
                   'typ': typ, 'narrative': narrative, 'questions': answers_box,
                   'question_content_id': question_content_id, 'word_number': word_number})
...
def standard_test(request, pk, pk1, pk2, pk3, pk4):
    questions1 = sorted(question.objects.filter(level_id=pk4, question_type_id__in=[2, 12, 3, 4, 5, 6]).order_by('-pk'),
    ...
    return render(request, "standard_test.html",
                  {"score": 0, "i": i, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4,
                   "programName": programName, "m": module1, "l": level1})
...
def ajax_standard_test(request, pk, pk1, pk2, pk3, pk4):
    ...
    if i == len(questions1):
        ...
        return render(request, "test_submit.html",
                      {"i": i, "score": s, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4,
                       "programName": programName, "test_name": "standard", "len": len(questions1)})
    ...
    if questions1[i].question_type.question_type == "Fill in the blanks":
        if (str(programName).lower() == "spoken english") and module1.module_name == "writing":
            ...
            random_questions = random.sample(list(level_questions),
                                             level_dict[str(module_level.objects.get(pk=pk4)).lower()])
            return render(request, "writing.html",
                          {"i": i, "score": s, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4,
                           "questions1": random_questions})
    ...
    if questions1[i].question_type.question_type == "Unscramble":
        ...
        return render(request, "jumbled_words.html",
                      {"len": range(0, len(strs.split())), "words": strs.split(),
                       "i": i, "score": s, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4,
                       'hint': 'none' if questions1[i].hint == '' else questions1[i].hint})
...
def av_test(request, pk, pk1, pk2, pk3, pk4, pk5, narrative):
    ...
    questions1 = question.objects.filter(level_id=pk4).filter(
        assessment_type=assessment_type.objects.get(assessment_type_id=pk5)).filter(narrative=narrative).order_by(
        '-question_content_id')
    ...
    return render(request, "av_test.html",
                  {"score": 0, "i": i, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4, "pk5": pk5,
                   "m": module, "l": level, 'narrative': narrative})
...
def ajax_av_test(request, pk, pk1, pk2, pk3, pk4, pk5, narrative):
    ...
    if c == 'av':
        ...
        return render(request, "test_submit.html",
                      {"i": i, "score": s, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4, "pk5": pk5,
                       "test_name": "av_test", "len": len(questions1), "narrative": narrative,
    ...
    if questions1[i].question_type.question_type == "Video":
        ...
        return render(request, "video.html",
                      {"i": i, "r": range(0, len(question_content)), "l": len(question_content), "score": s, "pk": pk,
                       "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4, "pk5": pk5, "narrative": narrative})
    ...
    if questions1[i].question_type.question_type == "Text":
        ...
        return render(request, "text.html",
                      {"q1": question_content, "i": i, "r": range(0, len(question_content)), "l": len(question_content),
                       "score": s, "pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4, "pk5": pk5,
                       "narrative": narrative})
...
def test_submit(request, pk, pk1, pk2, pk3, pk4):
    ...
    if test_name == "standard":
        s = student_status(student_id=student1, program_id=program1, module_id=module1, level_id=level1,
                           batch_id=batch1,
...
def writing_scores(request, pk, pk1, pk2, pk3, pk4):
    ...
    module1 = program_module.objects.get(pk=pk3)
    programName = program.objects.get(program_module=module1)
    return render(request, "writing_grading.html",
                  {"pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4, "programName": programName})
...
def writing_test_submit(request, pk, pk1, pk2, pk3, pk4, programName):
    ...
    if request.method == "POST":
        ...
        return render(request, "test_submit.html",
                      {"pk": pk, "pk1": pk1, "pk2": pk2, "pk3": pk3, "pk4": pk4, "score": score,
                       "programName": programName, "test_name": "standard", "len": len, "test_type": "writing",
                       "total_marks": total_marks, "level": level1})
...
def My_Strengths(request, pk, pk1, program):
    ...
    dat = []
    for i in old:
        dat.append(str(i.skillid))

    oldarr = json.dumps(dat)
    ...
    if request.method == 'POST':
        ...
        newdata = request.POST.getlist('sel')

        for i in newdata:
            obj.student_id = student.objects.get(pk=pk)
            obj.skillid = skillmaster.objects.get(skillid=i)
            obj.save()

        messages.success(
            request, 'Skills updated.',
            fail_silently=True,
        )
        old= skills.objects.filter(student_id=pk)
        dat = []
    ...
    return render(request, "Strengths.html",
                  {"pk": pk, "pk1": pk1, "program": program, "allskills": allskills,"old":oldarr })
